/*
	...
	Modified by Grahame for ARMA3/Epoch 1.0
	Script Version: 1.3.3_A3E
*/
...
#define SEARCH_CENTER epoch_centerMarkerPosition
#define SEARCH_RADIUS EPOCH_dynamicVehicleArea
...
call
{
	if (toLower worldName == "altis") exitWith {_blackList = [[[2092,14167],[10558,12505]]]; _heliStart = selectRandom [[1000.0,2.0],[3500.0,2.0],[5000.0,2.0],[7500.0,2.0],[9712.0,663.067],[12304.0,1175.07],[14736.0,2500.0],[16240.0,5000.0],[16240.0,7500.0],[16240.0,10000.0]];};
...
//	[nil,nil,rTitleText,format["A %1 is in distress! Watch for it and go to the crash site to secure the loot!",_porh], "PLAIN",10] call RE;
	...
	//["message", format[A %1 is in distress! Watch for it and go to the crash site to secure the loot!",_porh]] spawn EPOCH_serverCommand;
	...
	addToRemainsCollector [_crashwreck];
	...
	_helipilot 		= _aigroup createUnit ["CUP_C_C_Pilot_01",getPos _crashwreck,[],0,"FORM"];
...
//		[nil,nil,rTitleText,format["The %1 has crashed into the water, no loot can be secured",_porh], "PLAIN",10] call RE;
		//["message", format["The %1 has crashed into the water, no loot can be secured",_porh]] spawn EPOCH_serverCommand;
	}
	else
	{
		...
		_loot_amount = GUARANTEED_LOOT + (floor (random RANDOM_LOOT));

		if (DEBUG_MODE) then {diag_log(format["CRASHSPAWNER: Spawning %1 loot piles at crashsite at %2", _loot_amount, _pos]);};
		
		for "_crashloot" from 1 to _loot_amount do {
		
			_lootpos = [_pos,2,10,0,0,30,0] call BIS_fnc_findSafePos;
			
			_crate0 = createVehicle ["groundWeaponHolder", _lootpos, [], 0, "can_collide"];
					
			crashsite_loot call BIS_fnc_arrayshuffle;
			_lootObj = selectRandom crashsite_loot;
...
//		[nil,nil,rTitleText,format["The %1 has crashed, go and secure the loot!",_porh], "PLAIN",10] call RE;
		//["message", format["The %1 has crashed, go and secure the loot!",_porh]] spawn EPOCH_serverCommand;
...
//				[nil,nil,rTitleText,format["Survivors did not secure the %1 crash site!",_crashName], "PLAIN",10] call RE;
				//["message", format["Survivors did not secure the %1 crash site!",_crashName]] spawn EPOCH_serverCommand;
...
//					[nil,nil,rTitleText,format["Survivors have secured the crash site!"], "PLAIN",10] call RE;
					//["message", format["Survivors have secured the crash site!"]] spawn EPOCH_serverCommand;
